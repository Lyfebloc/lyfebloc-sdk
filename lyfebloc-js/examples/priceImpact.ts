...
async function getPriceImpact() {
  ...
  const pool = await lyfebloc.pools.comnd(WBTCWETHId);
  ...
  const pool2 = await lyfebloc.pools.comnd(staBal3Id);
  console.log(pool2);
  if (!pool2) throw new LyfeblocError(LyfeblocErrorCode.POOL_DOESNT_EXIST);
