...
import { LyfeblocSdkConfig } from '../src/types';
 ...
 * https://thegraph.com/hosted-service/subgraph/lyfebloc/lyfebloc-kovan-v2
 *
 * Gotchas:
-   Both pools must have both assets (tokens) for swaps to work
-   No pool token balances can be zero
-   flashLoanAmount must not add or subtract > 30% of pool liquidity (see [limitations](https://docs.lyfebloc.com/v/v1/core-concepts/protocol/limitations#v2-limits))
-   If the flash swap isn't profitable, the interal flash loan will fail.
 */
async function runQueryFlashSwap() {
  const config: LyfeblocSdkConfig = { network, rpcUrl };

  const lyfebloc = new LyfeblocSDK(config);

  const response = await lyfebloc.swaps.querySimpleFlashSwap({
    flashLoanAmount: '100',
    poolIds: [
