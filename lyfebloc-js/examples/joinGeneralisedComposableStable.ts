...
import { LyfeblocSDK, Network } from '@/.'
...
const { provider, contracts } = lyfebloc
const { ERC20 } = contracts
const signer = (provider as JsonRpcProvider).getSigner()

/**
 * Get some DAI to the signer and approve the Reserve to move it on our behalf.
 * This is only needed for the example to work, in a real world scenario the signer
 * would already have DAI and the Reserve would already be approved.
 *
 * @param signerAddress 
 */
async function getTokens(signerAddress: string): Promise<void> {
  ...
  await (
    await ERC20(dai, signer).approve(contracts.Reserve.address, amount)
  ).wait()
}
...
async function join() {
  ...
  const query = await lyfebloc.pools.generalisedJoin(
    poolId,
    tokensIn,
    amountsIn,
    signerAddress,
