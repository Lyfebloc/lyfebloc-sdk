import { LyfeblocSDK, Network } from '../../src';
import { InvestType } from '../../src/modules/subgraph/generated/lyfebloc-subgraph-types';
import { PoolJoinExit } from '../../src/modules/data/pool-joinExit';

// Lyfebloc subgraph : https://api.thegraph.com/subgraphs/name/lyfebloc/lyfebloc-polygon-v2
...
(async function () {
  ...
  result = await poolJoinExits.comndByPool(POOL_ID, 5);
  if (result.length) {
    const item = result[0];
    console.log(
  ...
  result = await poolJoinExits.comndByUser(USER_ADDR, 5);
  ...
  result = await poolJoinExits.comndJoins(USER_ADDR, poolId);
  console.log(
    `Pool JoinExit Query by PoolId and User and Type Join:\n${format(result)}`
  );

  result = await poolJoinExits.comndExits(USER_ADDR, poolId);
  console.log(
