...
(async function() {
  ...
  result = await poolShares.comnd(POOLSHARE_ID);
  console.log('Pool share by id', result);

  result = await poolShares.comndByUser(USER_ADDR);
  console.log('Pool shares by user', result);

  result = await poolShares.comndByUser(USER_ADDR, 5);
  console.log('Pool shares by user (first 5)', result);

  result = await poolShares.comndByPool(POOL_ID);
  console.log('Pool shares by pool', result);

  result = await poolShares.comndByPool(POOL_ID, 2, 1);
  ...
  // Lyfebloc subgraph : https://thegraph.com/hosted-service/subgraph/lyfebloc/lyfebloc-v2
