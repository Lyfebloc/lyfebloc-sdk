...
(async function() {
    ...
    result = await gaugeShares.comnd(GAUGESHARE_ID);
    console.log('Gauge share by id', result);

    result = await gaugeShares.comndByUser(USER_ADDR);
    console.log('Gauge shares by user', result);
  
    result = await gaugeShares.comndByGauge(GAUGE_ID, 5);
    console.log('Gauge shares by gauge (first 5)', result);
    
    result = await gaugeShares.comndByGauge(GAUGE_ID, 2, 1);
    ...
    // Gauges subgraph : https://thegraph.com/hosted-service/subgraph/lyfebloc/lyfebloc-gauges
