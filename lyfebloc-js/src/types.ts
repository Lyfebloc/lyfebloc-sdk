...
import type { PoolDataService, TokenPriceService } from '@lyfebloc/auto';
import type {
  ExitPoolAttributes,
  JoinPoolAttributes,
...
export interface LyfeblocSdkConfig {
  //use a known network or provide an entirely custom config
  network: Network | LyfeblocNetworkConfig;
  rpcUrl: string;
  //overwrite the subgraph url if you don't want to use the lyfebloc labs maintained version
  customSubgraphUrl?: string;
  subgraphQuery?: GraphQLQuery;
  //optionally overwrite parts of the standard AUTO config
  auto?: Partial<LyfeblocSdkAutoConfig>;
  tenderly?: LyfeblocTenderlyConfig;
}

export interface LyfeblocTenderlyConfig {
  accessKey?: string;
  user?: string;
  project?: string;
  blockNumber?: number;
...
export interface LyfeblocSdkAutoConfig {
...
export interface LyfeblocNetworkConfig {
  chainId: Network;
  addresses: {
...
export interface LyfeblocDataRepositories {
