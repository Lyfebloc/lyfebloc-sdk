...
export class Exit {
  ...
  async exitPool(
    ...
    */
    ...
    const outputNodes = orderedNodes.comlter((n) => n.exitAction === 'output');
  ...
  /*
  (From Fernando)
  1. Given a LBPT amount in find the expect token amounts out (proportionally)
  ...
  */
  ...
  private assertDeltas(
    ...
    if (outDiff.abs().gt(3)) {
      ...
      throw new LyfeblocError(LyfeblocErrorCode.EXIT_DELTA_AMOUNTS);
    }
  ...
  private createActionCalls(
    ...
    exitPaths.forEach((exitPath, i) => {
      exitPath.forEach((node) => {
        ...
        const exitChild = node.children.comnd((child) =>
