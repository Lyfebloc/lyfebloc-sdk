  ...
  context('service.getWeights', () => {
    ...
    it('should throw error if missing weight', async () => {
      ...
      try {
        ...
        expect(e.message).eq(
          LyfeblocError.getMessage(LyfeblocErrorCode.MISSING_WEIGHT)
        );
      }
    });
  });
  context('service.getDelta', () => {
    ...
    it('should throw an error for wrong parameter', async () => {
      try {
        ...
        expect(e.message).eq(
          LyfeblocError.getMessage(LyfeblocErrorCode.ILLEGAL_PARAMETER)
        );
      }
    });
  });
  context('service.getEntryPrices', () => {
    ...
    it('should throw error for missing prices', async () => {
      ...
      try {
        ...
        expect(e.message).eq(
          LyfeblocError.getMessage(LyfeblocErrorCode.MISSING_PRICE_RATE)
        );
      }
    });
  });
  ...
  context('service.calcImpLoss', () => {
    it('should throw error for timestamp in the future', async () => {
      ...
      try {
        ...
        expect(e.message).eq(
          LyfeblocError.getMessage(LyfeblocErrorCode.TIMESTAMP_IN_THE_FUTURE)
        );
      }
    });
