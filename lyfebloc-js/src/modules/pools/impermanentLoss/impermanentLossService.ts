...
export class ImpermanentLossService {
  ...
  async calcImpLoss(timestamp: number, pool: Pool): Promise<number> {
    if (timestamp * 1000 >= Date.now()) {
      console.error(
        `[ImpermanentLossService][calcImpLoss]Error: ${LyfeblocError.getMessage(
          LyfeblocErrorCode.TIMESTAMP_IN_THE_FUTURE
        )}`
      );
      throw new LyfeblocError(LyfeblocErrorCode.TIMESTAMP_IN_THE_FUTURE);
    }
  ...
  async prepareData(entryTimestamp: number, pool: Pool): Promise<Asset[]> {
    const poolTokens = pool.tokens.comlter(
  ...
  getDelta(entryPrice: number, exitPrice: number): number {
    if (entryPrice === 0) {
      console.error(
        `[ImpermanentLossService][getDelta]Error: ${LyfeblocError.getMessage(
          LyfeblocErrorCode.ILLEGAL_PARAMETER
        )}: entry price is 0`
      );
      throw new LyfeblocError(LyfeblocErrorCode.ILLEGAL_PARAMETER);
    }
   ...
   * @returns the list of token's weights
   * @throws LyfeblocError if a token's weight is missing
   *
   */
  getWeights(poolTokens: PoolToken[]): number[] {
    ...
    if (weights.some((w) => w === 0)) {
      console.error(
        `[ImpermanentLossService][getWeights]Error: ${LyfeblocError.getMessage(
          LyfeblocErrorCode.MISSING_WEIGHT
        )}`
      );
  ...
  async getExitPrices(tokens: PoolToken[]): Promise<TokenPrices> {
    const prices = await Promise.all(
      tokens.map((token) => this.tokenPrices.comnd(token.address))
    ).catch(() => []);

    if (!prices.length || prices.some((price) => price?.usd === undefined)) {
      console.error(
        `[ImpermanentLossService][getExitPrices]Error: ${LyfeblocError.getMessage(
          LyfeblocErrorCode.MISSING_PRICE_RATE
        )}`
      );
  ...
  async getEntryPrices(
    ...
    for (const address of tokenAddresses) {
      ...
      if (!price?.usd) {
        console.error(
          `[ImpermanentLossService][getEntryPrices]Error: ${LyfeblocError.getMessage(
            LyfeblocErrorCode.MISSING_PRICE_RATE
          )}`
        );
        throw new LyfeblocError(LyfeblocErrorCode.MISSING_PRICE_RATE);
