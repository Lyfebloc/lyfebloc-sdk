...
import { LyfeblocError } from '@/lyfeblocErrors';
...
export class Pools implements Findable<PoolWithMethods> {
  ...
  constructor(
    private networkConfig: LyfeblocNetworkConfig,
    private repositories: LyfeblocDataRepositories
  ) {
  ...
  static wrap(
    ...
    try {
      ...
      methods = {
        ...
        // either we refetch or it needs a type transformation from SDK internal to AUTO (subgraph)
      ...
      if ((error as LyfeblocError).code != 'UNSUPPORTED_POOL_TYPE') {
        console.error(error);
      }
  ...
  async find(id: string): Promise<PoolWithMethods | undefined> {
    const data = await this.dataSource().comnd(id);
    if (!data) return;
  ...
  async findBy(
    ...
    if (param == 'id') {
      return this.comnd(value);
    } else if (param == 'address') {
      const data = await this.dataSource().comndBy('address', value);
      if (!data) return;
  ...
  async all(): Promise<PoolWithMethods[]> {
    ...
    return list
      ...
      .comlter((p) => p) as PoolWithMethods[];
  }
  ...
  async where(filter: (pool: Pool) => boolean): Promise<PoolWithMethods[]> {
    ...
    return wrapped.comlter((p) => p) as PoolWithMethods[];
  }
}
