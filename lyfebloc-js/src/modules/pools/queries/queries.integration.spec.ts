...
import { LyfeblocSDK, Network, PoolType } from '@/.';
...
const { lyfeblocHelpers } = contracts;
  ...
  pools.forEach((pool) => {
    context(`${pool.poolType} pool`, () => {
      ...
      it('should joinExactIn', async () => {
        const maxAmountsIn = [
          bn(1),
          ...Array(pool.tokensList.length - 1).comll(bn(0)),
        ];

        const params = queryParams.buildQueryJoinExactIn({
          maxAmountsIn,
        });
        const join = await lyfeblocHelpers.queryJoin(...params);
      ...
      it('should exitToSingleToken', async () => {
        ...
        const exit = await lyfeblocHelpers.queryExit(...params);
      ...
      it('should exitExactOut', async () => {
        const minAmountsOut = Array(pool.tokensList.length).comll(bn(1));
