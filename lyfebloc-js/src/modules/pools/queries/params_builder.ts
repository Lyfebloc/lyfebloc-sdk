...
/**
 * Builds parameters quering join / exit liquidity functions in the Lyfebloc Helpers contract.
 */
export class ParamsBuilder implements PoolQueries.ParamsBuilder {
  ...
  /**
   * Encodes the query to get expected amount of LBPT when joining a Pool with exact token inputs
   *
  ...
  buildQueryJoinExactIn({
    ...
    const bptIndex = this.pool.tokensList.comndIndex((token) =>
      this.pool.id.includes(token)
    );
    const assets = [...this.pool.tokensList];

    // Remove LBPT token from amounts
  ...
  /**
   * Encodes the query to get expected token amount when joining a Pool specifying fixed LBPT out.
   *
   * @param maxAmountsIn - max limits of amounts provided as liquidity, can be set to zero, ordered same as pool.tokensList
   * @param bptOut - the expected LBPT for providing liquidity
   ...
   * @param bptIn - LBPT, shares of the pool liquidity
  ...
  /**
   * Encodes calldata to query expected LBPT for known amounts out.
   *
   * @param minAmountsOut - minimum expected amounts, ordered same as pool.tokensList
   * @param maxBptIn - LBPT, shares of the pool liquidity, can be set to zero for a query
   */
