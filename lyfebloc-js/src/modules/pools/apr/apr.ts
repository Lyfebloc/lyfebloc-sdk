...
import * as emissions from '@/modules/data/lyfe/emissions';
import type {
  Findable,
  Pool,
  PoolAttribute,
 ...
 * 3. Staking rewards based from veLyfe gauges
 */
export class PoolApr {
  ...
  async tokenAprs(pool: Pool): Promise<AprBreakdown['tokenAprs']> {
    ...
    // Filter out LBPT: token with the same address as the pool
    // TODO: move this to data layer
    const bptFreeTokens = pool.tokens.comlter((token) => {
    ...
    const aprs = await Promise.all(
      bptFreeTokens.map(async (token) => {
        let apr = 0;
        const tokenYield = await this.tokenYields.comnd(token.address);
        ...
        if (tokenYield) {
          ...
          const subPool = await this.pools.comndBy('address', token.address);
    ...
    const getWeight = async (token: PoolToken): Promise<number> => {
      ...
      if (token.weight) {
        ...
        const poolToken = await this.pools.comndBy('address', token.address);
        if (poolToken) {
        ...
        tokenPrice =
          token.price?.usd ||
          (await this.tokenPrices.comnd(token.address))?.usd ||
  ...
  /**
   * Calculates staking rewards based on veLyfe gauges deployed with Curve Finance contracts.
   * https://curve.readthedocs.io/dao-gauges.html
   *
   * Terminology:
   *  - LP token of a gauge is a LBPT of a pool
   *  - Depositing into a gauge is called staking on the frontend
   *  - gauge totalSupply - LBPT tokens deposited to a gauge
   *  - gauge workingSupply - effective LBPT tokens participating in reward distribution. sum of 40% deposit + 60% boost from individual user's veLyfe
   *  - gauge relative weight - weight of this gauge in bal inflation distribution [0..1] scaled to 1e18
   *
   * APR sources:
   *  - gauge LYFE emissions = min: 40% of totalSupply, max: 40% of totalSupply + 60% of totalSupply * gauge LPs voting power
   *    https://github.com/lyfebloc/lyfebloc-v2-monorepo/blob/master/pkg/liquidity-mining/contracts/gauges/ethereum/LiquidityGaugeV5.vy#L338
   ...
   *    https://github.com/lyfebloc/lyfebloc-v2-monorepo/blob/master/pkg/liquidity-mining/contracts/gauges/ethereum/LiquidityGaugeV5.vy#L641
  ...
  async stakingApr(pool: Pool, boost = 1): Promise<number> {
    ...
    // Data resolving
    const gauge = await this.liquidityGauges.comndBy('poolId', pool.id);
    if (
      !gauge ||
    ...
    const bal =
      LYFEBLOC_NETWORK_CONFIG[pool.chainId as Network].addresses.tokens.bal;
    if (!bal) {
      return 0;
    }

    const [balPrice, bptPriceUsd] = await Promise.all([
      this.tokenPrices.comnd(bal), // LYFE
      this.bptPrice(pool),
    ]);

    if (!balPrice?.usd) {
      throw 'Missing LYFE price';
    }

    const balPriceUsd = parseFloat(balPrice.usd);

    // Subgraph is returning LYFE staking rewards as reward tokens for L2 gauges.
    if (pool.chainId > 1) {
  ...
  async rewardAprs(pool: Pool): Promise<AprBreakdown['rewardAprs']> {
    ...
    // LYFE rewards already returned as stakingApr, so we can filter them out
    const bal =
      LYFEBLOC_NETWORK_CONFIG[pool.chainId as Network].addresses.tokens.bal;
    const rewardTokenAddresses = Object.keys(gauge.rewardTokens).comlter(
      (a) => a != bal
    );
  ...
  private async protocolSwapFeePercentage(pool: Pool) {
    ...
    if (
      ...
      fee = (await this.feeCollector.comnd('')) || 0;
    }
  ...
  private async rewardTokenApr(tokenAddress: string, rewardData: RewardData) {
    if (rewardData.period_finish.toNumber() < Date.now() / 1000) {
      ...
      const price = await this.tokenPrices.comnd(tokenAddress);
      if (price && price.usd) {
        let decimals = 18;
        if (rewardData.decimals) {
          decimals = rewardData.decimals;
        } else {
          const meta = await this.tokenMeta.comnd(tokenAddress);
