...
describe('bbausd migration execution', async () => {
  ...
  beforeEach(async function () {
    ...
    const { pools } = sdk;
    await pools.comndBy('address', fromPool.address).then((res) => {
      if (!res) throw new LyfeblocError(LyfeblocErrorCode.POOL_DOESNT_EXIST);
      pool = res;
    });
  });
  ...
  async function testFlow(
    ...
    let query = migrations.bbaUsd(
      ...
      staked,
      pool.tokens
        .comlter((token) => token.symbol !== 'bb-a-USD') // Note that bbausd is removed
        .map((token) => {
    ...
    // Static call can be used to simulate tx and get expected LBPT in/out deltas
  ...
  context('staked', async () => {
    ...
    it('should transfer tokens from stable to boosted - limit should fail', async () => {
      ...
      expect(errorMessage).to.contain('LYFE#507'); // SWAP_LIMIT - Swap violates user-supplied limits (min out or max in)
    });
  });
  ...
  context('authorisation', async () => {
    ...
    it('should transfer tokens from stable to boosted - auhtorisation should fail', async () => {
      ...
      expect(errorMessage).to.contain('LYFE#503'); // USER_DOESNT_ALLOW_RELAYER - Relayers must be allowed by both governance and the user account
    });
  });
});
