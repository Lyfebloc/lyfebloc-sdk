...
export class StablesBuilder {
   ...
   * @param bptIn Amount of LBPT tokens to migrate.
   * @param minBptOut Minimum of expected LBPT out ot the migration flow.
   * @param staked Indicates whether tokens are initially staked or not.
   * @param underlyingTokens Underlying token addresses. Array must have the same length and order as underlying tokens in pool being migrated from. Refer to [getPoolTokens](https://github.com/lyfebloc/lyfebloc-v2-monorepo/blob/master/pkg/interfaces/contracts/Reserve/IReserve.sol#L334).
   ...
   * @param amount Amount of LBPT to exit with.
  ...
  buildExit(
    ...
    const minAmountsOut = Array<string>(underlyingTokens.length).comll('0');
  ...
  /**
   * Creates encoded batchSwap function to swap stables to new phantom stable pool LBPT.
   * outputreferences should contain the amount of resulting LBPT.
   *
   * @param expectedBptReturn LBPT amount expected out of the swap.
  ...
  /**
   * Uses relayer to withdraw staked LBPT from gauge and send to relayer
   *
   * @param sender Sender address.
   * @param amount Amount of LBPT to exit with.
  ...
  /**
   * Uses relayer to deposit user's LBPT to gauge and sends to recipient
   *
   * @param recipient Recipient address.
   * @param gaugeAddress Gauge address.
