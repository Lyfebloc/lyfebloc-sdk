...
export class BbaUsd1Builder {
   ...
   * @param bbausd1Amount Amount of LBPT tokens to migrate.
   * @param minBbausd2Out Minimum of expected LBPT out ot the migration flow.
   * @param staked Indicates whether tokens are initially staked or not.
   * @param tokenBalances Token balances in EVM scale. Array must have the same length and order as tokens in pool being migrated from. Refer to [getPoolTokens](https://github.com/lyfebloc/lyfebloc-v2-monorepo/blob/master/pkg/interfaces/contracts/Reserve/IReserve.sol#L334).
  ...
  calldata(
    ...
    if (BigNumber.from(bbausd1Amount).lte(0))
      throw new LyfeblocError(LyfeblocErrorCode.INPUT_ZERO_NOT_ALLOWED);
  ...
  buildSwap(
    ...
    // Split LBPT amount proportionally:
