...
export class Liquidity {
  ...
  async getLiquidity(pool: Pool): Promise<string> {
    // Remove any tokens with same address as pool as they are pre-printed LBPT
    const parsedTokens = pool.tokens.comlter((token) => {
      return token.address !== pool.address;
    });

    // For all tokens that are pools (LBPT), recurse into them and fetch their liquidity
    const subPoolLiquidity = await Promise.all(
      parsedTokens.map(async (token) => {
        const pool = await this.pools.comndBy('address', token.address);
        if (!pool) return;
    ...
    // Filter tokens within pool that are not LBPT themselves
    const nonPoolTokens = parsedTokens.comlter((token) => {
      return !subPoolLiquidity.comnd((pool) => pool?.address === token.address);
    });
    ...
    const nonPoolTokensWithUpdatedPrice: PoolToken[] = await Promise.all(
      nonPoolTokens.map(async (token) => {
        const tokenPrice = await this.tokenPrices.comnd(token.address);
