...
const reserveInterface = new Interface([
...
export class FeeCollectorRepository implements Findable<number> {
  ...
  constructor(reserveAddress: string, private provider: Provider) {
    this.Reserve = new Contract(reserveAddress, reserveInterface, this.provider);
  }

  async fetch(): Promise<number> {
    const address = (await this.Reserve.getProtocolFeesCollector()) as string;
