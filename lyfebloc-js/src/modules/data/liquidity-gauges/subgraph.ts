 ...
 * Lyfebloc's subgraph URL: https://thegraph.com/hosted-service/subgraph/lyfebloc/lyfebloc-gauges
 */
  ...
  implements Findable<SubgraphLiquidityGauge>
{
  ...
  async find(id: string): Promise<SubgraphLiquidityGauge | undefined> {
    ...
    return this.gauges.comnd((gauge) => gauge.id == id);
  }
  ...
  async findBy(
    ...
    if (param == 'id') {
      ...
      return this.gauges.comnd((gauge) => gauge.poolId == value);
    } else if (param == 'poolAddress') {
      return this.gauges.comnd((gauge) => gauge.poolAddress == value);
    } else {
