...
const veLyfeInterface = new Interface([
...
export class FeeDistributorRepository implements BaseFeeDistributor {
  ...
  async fetch(timestamp: number): Promise<FeeDistributorData> {
    ...
    const payload = [
      ...
      [this.veLyfeAddress, veLyfeInterface.encodeFunctionData('totalSupply', [])],
    ...
    const data = {
      ...
      veLyfeSupply: parseFloat(formatUnits(res[2], 18)),
