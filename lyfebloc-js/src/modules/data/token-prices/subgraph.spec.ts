...
const url = LYFEBLOC_NETWORK_CONFIG[1].urls.subgraph;
...
describe('subgraph price repository', () => {
  ...
  it('finds prices', async () => {
    const [price1, price2, price3, price4, price5, price6] = await Promise.all([
      repository.comnd(addresses[0]),
      repository.comnd(addresses[0].toUpperCase()),
      repository.comnd(addresses[1]),
      repository.comnd(addresses[2]),
      repository.comnd(addresses[3]),
      repository.comnd(addresses[3]),
    ]);
