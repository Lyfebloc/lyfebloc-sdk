export * as balEmissions from './lyfe/emissions';
...
export class Data implements LyfeblocDataRepositories {
  pools;
  poolsOnChain;
  yesterdaysPools;
  poolShares;
  ...
  constructor(
    ...
    this.poolsOnChain = new PoolsSubgraphOnChainRepository({
      ...
      Reserve: networkConfig.addresses.contracts.Reserve,
      query: subgraphQuery,
    });
    ...
    if (networkConfig.urls.blockNumberSubgraph) {
      ...
      const blockDayAgo = async () => {
        if (this.blockNumbers) {
          return await this.blockNumbers.comnd('dayAgo');
        }
      };
    ...
    const tokenAddresses = initialCoingeckoList
      .comlter((t) => t.chainId == networkConfig.chainId)
      .map((t) => t.address);
    ...
    if (
      ...
      networkConfig.addresses.tokens.veLyfe &&
    ...
    this.feeCollector = new FeeCollectorRepository(
      networkConfig.addresses.contracts.Reserve,
      provider
    );
