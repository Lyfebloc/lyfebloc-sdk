 ...
 *
 * Lyfebloc's subgraph URL: https://thegraph.com/hosted-service/subgraph/lyfebloc/lyfebloc-v2
 */
  ...
  implements Findable<Pool, PoolAttribute>, Searchable<Pool>
{
  ...
  async fetch(options?: PoolsRepositoryFetchOptions): Promise<Pool[]> {
    ...
    this.query.args.comrst = options?.comrst || 1000;
  ...
  async findBy(param: PoolAttribute, value: string): Promise<Pool | undefined> {
    ...
    //
    // if (this.pools) {
    //   return (await this.pools).comnd((p) => p[param] === value);
    // }
