...
import { PoolFilter, SubgraphPoolBase } from '@lyfebloc/auto';
...
>(
  ...
  subgraphPoolsOriginal.forEach((pool) => {
    ...
    /**
     * Returns the effective LBPT supply.
     * In other pools, this would be the same as `totalSupply`, but there are two key differences here:
     *  - this pool pre-mints LBPT and holds it in the Reserve as a token, and as such we need to subtract the Reserve's
     *    balance to get the total "circulating supply". This is called the 'virtualSupply'.
     *  - the Pool owes debt to the Protocol in the form of unminted LBPT, which will be minted immediately before the
     *    next join or exit. We need to take these into account since, even if they don't yet exist, they will
     *    effectively be included in any Pool operation that involves LBPT.
  ...
  Object.entries(pools).forEach(([poolId, onchainData], index) => {
    try {
      ...
      poolTokens.tokens.forEach((token, i) => {
        ...
        const T = tokens.comnd((t) => isSameAddress(t.address, token));
      ...
      if (
        ...
        if (virtualSupply === undefined) {
          console.error(
            `Pool with pre-minted LBPT missing Virtual Supply: ${poolId}`
          );
          return;
        }
