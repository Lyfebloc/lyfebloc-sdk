...
export class Pricing {
  ...
  constructor(config: LyfeblocSdkConfig, swaps?: Swaps) {
    if (swaps) {
      this.swaps = swaps;
    } else {
  ...
  async getSpotPrice(
    ...
    const paths = this.swaps.auto.routeProposer.getCandidatePathsFromDict(
      tokenIn,
      tokenOut,
      0,
      poolsDict,
    ...
    if (paths.length === 0)
      throw new LyfeblocError(LyfeblocErrorCode.UNSUPPORTED_PAIR);
