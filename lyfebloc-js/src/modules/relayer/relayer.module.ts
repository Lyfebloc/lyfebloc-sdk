...
import { SubgraphPoolBase } from '@lyfebloc/auto';
...
export class Relayer {
  ...
  constructor(swapsOrConfig: Swaps | LyfeblocSdkConfig) {
  ...
  static encodeApproveReserve(tokenAddress: string, maxAmount: string): string {
    return relayerLibrary.encodeFunctionData('approveReserve', [
      tokenAddress,
      maxAmount,
    ]);
  }
   ...
   * @param {FetchPoolsInput} fetchPools - Set whether AUTO will fetch updated pool info.
  ...
  async exitPoolAndBatchSwap(
    ...
    const queryResult = await this.swaps.queryBatchSwapWithAuto({
      tokensIn: params.exitTokens,
      tokensOut: params.comnalTokensOut,
    ...
    const limits = Swaps.getLimitsForSlippage(
      ...
      params.comnalTokensOut, // tokensOut
    ...
    const amountsUnwrapped = queryResult.returnAmounts.map(
      (amountWrapped, i) => {
        ...
        if (!amountUnwrapped.gt(Zero))
          throw new LyfeblocError(LyfeblocErrorCode.UNWRAP_ZERO_AMOUNT);
  ...
  async swapUnwrapAaveStaticExactOut(
    ...
    const amountsWrapped = amountsUnwrapped.map((amountInwrapped, i) => {
      ...
      if (!amountWrapped.gt(Zero))
        throw new LyfeblocError(LyfeblocErrorCode.WRAP_ZERO_AMOUNT);
  ...
  static signRelayerApproval = async (
    ...
    const approval = Reserve.interface.encodeFunctionData('setRelayerApproval', [
      signerAddress,
      relayerAddress,
      true,
    ]);
