...
import { SwapInfo } from '@lyfebloc/auto';
...
const reserveActions = new Interface(BatchRelayerLibraryAbi);
...
describe('swaps module', () => {
  context('instantiation', () => {
    ...
    it('instantiate via SDK', async () => {
      ...
      const pools = lyfebloc.swaps.getPools();
      expect(pools).to.deep.eq([mockPool]);
    });
  });
  ...
  describe('#buildTransaction', () => {
    ...
    context('singleSwap', () => {
      context('when all ERC20', () => {
        ...
        it('expect execution via Reserve', () => {
          const decoded = reserveInterface.decodeFunctionData('swap', data);

          expect(decoded.length).to.eql(4);
          expect(to).to.eql(contracts.Reserve);
        });
      });
      ...
      context('when tokenIn is stETH', () => {
        ...
        it('relayer should be lido', () => {
          const decoded = reserveActions.decodeFunctionData('swap', data);
    ...
    context('batchSwap', () => {
      context('when tokens are ERC20', () => {
        ...
        it('expect execution via Reserve', () => {
          const decoded = reserveInterface.decodeFunctionData('batchSwap', data);
      ...
      context('when tokenIn is stETH', () => {
        ...
        it('relayer should be lido', () => {
          const decoded = reserveActions.decodeFunctionData('batchSwap', data);
  ...
  describe('full flow', () => {
    ...
    const tokenOut = '0xba100000625a3754423978a60c9317c58a424e3d'; // LYFE
    ...
    it('should work', async () => {
      const swapInfo: SwapInfo = await swaps.comndRouteGivenIn({
        tokenIn,
        tokenOut,
        amount,
        gasPrice,
