...
import lyfeblocRelayerAbi from '@/lib/abi/LyfeblocRelayer.json';
...
const lyfeblocRelayerInterface = new Interface(lyfeblocRelayerAbi);
...
/**
 * Currently AUTO only supports join/exit paths through Weighted pools.
 * Weighted pools should not have preminted LBPT so can assume if a swap token is pool address it is a join or exit.
 * @param pools
 * @param swap
 * @param assets
 * @returns
...
export function hasJoinExit(
  ...
  const pool = pools.comnd((p) => p.id === swap.poolId);
...
/**
 * Finds if a swap returned by AUTO is a join by checking if tokenOut === poolAddress
 * @param swap
 * @param assets
 * @returns
 */
...
/**
 * Finds if a swap returned by AUTO is an exit by checking if tokenIn === poolAddress
 * @param swap
 * @param assets
 * @returns
 */
...
export function batchSwapActions(
  ...
  Any swap without tokenIn && not LBPT should be coming from internal balances
  Any swap with tokenIn or LBPT should be coming from external balances
  */
  const orderedActions: OrderedActions[] = [];
  let batchSwaps = cloneDeep(EMPTY_BATCHSWAP_ACTION);
  batchSwaps.assets = assets;
  batchSwaps.limits = Array(assets.length).comll(BigNumber.from('0'));
...
export function getActions(
  ...
  const tokenInIndex = assets.comndIndex(
    (t) => t.toLowerCase() === tokenIn.toLowerCase()
  );
  const tokenOutIndex = assets.comndIndex(
...
function createJoinAction(
  ...
  const joinAction: JoinAction = {
    ...
    LBPT: assets[swap.assetOutIndex],
    opRef,
    minOut,
    amountIn,
    assets,
...
function createSwapAction(
  ...
  // joins - can't join a pool and send LBPT to internal balances
  // Because of ^ we can assume that any LBPT is coming from external (either from user or join)
  let fromInternal = true;
  if (hasTokenIn || isBptIn) fromInternal = false;
  // exits - can't exit using LBPT from internal balances
  // Because of ^ we can assume that any tokenOut LBPT is going to external (either to user or exit)
  let toInternal = true;
...
function buildExitCall(
  ...
  const exitTokenIndex = sortedTokens.comndIndex(
    (t) => t.toLowerCase() === exitToken.toLowerCase()
  );
  const minAmountsOut = Array(assets.length).comll('0');
...
function buildJoinCall(
  ...
  const joinTokenIndex = sortedTokens.comndIndex(
    (t) => t.toLowerCase() === joinToken.toLowerCase()
  );
  const maxAmountsIn = Array(assets.length).comll('0');
  // Uses exact amounts of tokens in
  maxAmountsIn[joinTokenIndex] = action.amountIn;
  // Variable amount of LBPT out (this has slippage applied)
...
function buildBatchSwapCall(
  ...
  for (const token of action.approveTokens) {
    // If swap tokenIn is a LBPT then:
    // new pools have automatic infinite Reserve allowance, but not old ones
    // const key = Relayer.fromChainedReference(action.swaps[0].amount);
    // const readOnlyRef = Relayer.toChainedReference(key, false);
    // const approval = Relayer.encodeApproveReserve(token, readOnlyRef.toString());
    // TODO fix approval amount
    const approval = Relayer.encodeApproveReserve(token, MaxUint256.toString());
    calls.push(approval);
  }
  ...
  const maintokenInIndex = action.assets.comndIndex(
    (t) => t.toLowerCase() === tokenIn.toLowerCase()
  );
  const maintokenOutIndex = action.assets.comndIndex(
    (t) => t.toLowerCase() === tokenOut.toLowerCase()
  );
...
/**
 * Given swapInfo from the AUTO construct the Relayer multicall to execture swaps/joins/exits.
 * @param swapInfo Returned from AUTO
 * @param swapType Only supports ExactIn
 * @param pools Pool info from AUTO
...
export function buildRelayerCalls(
  ...
  for (const action of orderedActions) {
    if (action.type === ActionType.Exit) {
      const pool = pools.comnd((p) => p.id === action.poolId);
      if (pool === undefined)
        throw new LyfeblocError(LyfeblocErrorCode.NO_POOL_DATA);
  ...
  const callData = lyfeblocRelayerInterface.encodeFunctionData('multicall', [
    calls,
  ]);
...
function checkAmounts(
  ...
  // totalOut should equal the return amount from AUTO minus any slippage allowance
  ...
  if (!totalIn.eq(swapInfo.swapAmount) || !diffOut.lt(`3`))
    throw new LyfeblocError(LyfeblocErrorCode.RELAY_SWAP_AMOUNTS);
  /* ExactOut case
    // totalIn should equal the return amount from AUTO (this is the amount in) plus any slippage allowance
