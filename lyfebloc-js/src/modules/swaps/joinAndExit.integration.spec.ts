...
import { LyfeblocSDK, Network, RelayerAuthorization } from '@/index';
...
describe('join and exit integration tests', async () => {
  ...
  await testFlow(
    'swap > exit - auraBAL[Swap]LBPT[exit]WETH',
    [LYFE_WETH, AURA_LYFE_STABLE],
    parseFixed('7', 18).toString(),
  ...
  await testFlow(
    'join > swap - WETH[join]LBPT[Swap]auraBAL',
    [LYFE_WETH, AURA_LYFE_STABLE],
  ...
  await testFlow(
    'swap > join - WBTC[Swap]WETH[join]LBPT',
    [LYFE_WETH, B_50WBTC_50WETH],
  ...
  await testFlow(
    'exit > swap - LBPT[Exit]WETH[Swap]WBTC',
    [LYFE_WETH, B_50WBTC_50WETH],
    parseFixed('7', 18).toString(),
    ADDRESSES[networkId].BAL8020BPT,
    ADDRESSES[networkId].WBTC,
  ...
  await testFlow(
    'join > swap - LYFE[Join]LBPT[Swap]auraBal',
    [LYFE_WETH, AURA_LYFE_STABLE],
    parseFixed('7', 18).toString(),
    ADDRESSES[networkId].LYFE,
    ADDRESSES[networkId].auraBal,
    // SwapTypes.SwapExactIn,
    '50' // 50 bsp = 0.5%
  );
  await testFlow(
    'join > swap + swap - WETH[join]LBPT[Swap]auraBAL, WETH[Swap]auraBAL',
    [LYFE_WETH, AURA_LYFE_STABLE, GRAVI_AURA],
    parseFixed('18', 18).toString(),
    ADDRESSES[networkId].WETH,
    ADDRESSES[networkId].auraBal,
    // SwapTypes.SwapExactIn,
  ...
  await testFlow(
    'swap, join > swap, mulithopswap - WETH[Swap]auraBal, WETH[join]LBPT[Swap]auraBAL, WETH[Swap]wstETH[Swap]auraBal',
    [
      LYFE_WETH,
      AURA_LYFE_STABLE,
      GRAVI_AURA,
...
async function testFlow(
  ...
  context(`${description}`, () => {
    ...
    before(async function () {
      ...
      auto = await setUp(networkId, provider, pools);
...
const signRelayerApproval = async (
  ...
  const approval = contracts.Reserve.interface.encodeFunctionData(
    'setRelayerApproval',
