  ...
  context('ExactOut', () => {
    ...
    context('same receiver as signer', () => {
      before(async function () {
        ...
        const swapInfo: SwapInfo = await swaps.comndRouteGivenOut({
          tokenIn,
          tokenOut,
          amount: amountOut,
          gasPrice,
    ...
    context('in mempool', () => {
      ...
      it('fails on slippage', async () => {
        ...
        try {
          ...
          // https://github.com/lyfebloc/lyfebloc-v2-monorepo/blob/master/pkg/solidity-utils/contracts/helpers/LyfeblocErrors.sol#L218
          reason = err.reason;
        }

        expect(reason).to.contain('LYFE#507');
