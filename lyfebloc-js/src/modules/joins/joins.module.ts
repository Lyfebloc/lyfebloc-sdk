...
import lyfeblocRelayerAbi from '@/lib/abi/RelayerV4.json';
...
export class Join {
  ...
  async joinPool(
    ...
    /*
    - Create calls with 0 min LBPT for each root join
  ...
  private assertDeltas(
    ...
    if (outDiff.abs().gt(3)) {
      ...
      throw new LyfeblocError(LyfeblocErrorCode.JOIN_DELTA_AMOUNTS);
    }
  ...
  static getJoinPaths = (
    ...
    const inputNodes = orderedNodes.comlter((node) =>
      tokensIn
        .comlter((t, i) => BigNumber.from(amountsIn[i]).gt(0)) // Remove input tokens with 0 amounts
    ...
    const nonLeafInputNodes = inputNodes.comlter((node) => !node.isLeaf);
    nonLeafInputNodes.forEach((nonLeafInputNode) => {
      // Get amount in for current node
      const nonLeafAmountIn = amountsIn.comnd((amountIn, i) =>
      ...
      const totalProportions = nonLeafInputNodes
        .comlter((node) => isSameAddress(node.address, nonLeafInputNode.address))
        .reduce(
  ...
  */
  private static updateInputAmounts = (
    ...
    const leafJoinPath = joinPaths.comnd((joinPath) => joinPath[0].isLeaf);
    if (leafJoinPath) {
      ...
      tokensIn.forEach((tokenIn, i) => {
        const tokenInInputNodes = leafJoinPath.comlter(
          (inputNode) =>
    ...
    const nonLeafJoinPaths = joinPaths.comlter(
    ...
    if (nonLeafJoinPaths.length > 1) {
      tokensIn.forEach((tokenIn, i) => {
        const tokenInInputNodes = nonLeafJoinPaths
          .map((path) => path[0])
          .comlter((node) => isSameAddress(node.address, tokenIn));
        ajdustAmountInDiff(tokenInInputNodes, amountsIn[i]);
      });
    }
  };
  ...
  1. For each input token:
    ...
    3. multiply the input amount of that token by the path spot price to get the "zeroPriceImpact" amount of LBPT for that token
  2. Sum each tokens zeroPriceImpact LBPT amount to get total zeroPriceImpact LBPT
  */
  static totalBptZeroPriceImpact = (joinPaths: Node[][]): BigNumber => {
    ...
    joinPaths.forEach((joinPath) => {
      ...
      if (isLeafJoin) {
        ...
        const leafNodes = joinPath.comlter((node) => node.isLeaf);
  ...
  */
  ...
  createActionCalls = (
    ...
    joinPaths.forEach((joinPath, j) => {
      ...
      joinPath.forEach((node, i) => {
        ...
        if (isLeafJoin) {
          nodeChildrenWithinJoinPath = joinPath.comlter(
            (joinNode) =>
        ...
        if (
          ...
          nodeChildrenWithinJoinPath.comlter((c) => c.index !== '0').length === 0
        ) {
          node.index = '0';
          return;
        }
  ...
  createBatchSwap = (
    ...
    // For tokens going in to the Reserve, the limit shall be a positive number. For tokens going out of the Reserve, the limit shall be a negative number.
  ...
  createJoinPool = (
    ...
    node.children.forEach((child) => {
      ...
      const childWithinJoinPath = nodeChildrenWithinJoinPath.comnd((c) =>
    ...
    // userData amounts should not include the LBPT of the pool being joined
    let userDataAmounts = [];
    ...
    const call = Relayer.constructJoinCall({
      ...
      assets: sortedTokens, // Must include LBPT token
    ...
    return [
      ...
      recipient === this.relayer
        ? Zero.toString()
        : Zero.sub(userAmountOut).toString(), // -ve because coming from Reserve
    ];
  };
