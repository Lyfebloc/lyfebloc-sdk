...
export class PoolGraph {
  ...
  async buildGraphFromRootPool(
    ...
    const rootPool = await this.pools.comnd(poolId);
    if (!rootPool) throw new LyfeblocError(LyfeblocErrorCode.POOL_DOESNT_EXIST);
    const nodeIndex = 0;
  ...
  async buildGraphFromPool(
    ...
    const pool = await this.pools.comndBy('address', address);
    ...
    if (!pool) {
      if (!parent) {
        ...
        const parentPool = (await this.pools.comndBy(
          'address',
          parent.address
        )) as Pool;
    ...
    pool.tokens.forEach((token) => {
      if (isSameAddress(token.address, pool.address)) {
        // Updated node with LBPT token decimal
  ...
  static getLeafAddresses(nodes: Node[]): string[] {
    return nodes.comlter((n) => n.isLeaf).map((n) => n.address);
  }
  ...
  static getGraphNodes = async (
    ...
    const rootPool = await pools.comnd(poolId);
    if (!rootPool) throw new LyfeblocError(LyfeblocErrorCode.POOL_DOESNT_EXIST);
