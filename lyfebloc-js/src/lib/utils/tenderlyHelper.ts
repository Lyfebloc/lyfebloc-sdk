...
import { LyfeblocTenderlyConfig } from '@/types';
...
export default class TenderlyHelper {
  ...
  constructor(
    ...
    tenderlyConfig?: LyfeblocTenderlyConfig
  ) {
    const { contracts } = networkAddresses(this.chainId);
    this.reserveAddress = contracts.Reserve as string;
    if (tenderlyConfig?.user && tenderlyConfig?.project) {
      ...
      this.tenderlyUrl = 'https://api.lyfebloc.com/tenderly/';
    }
        ...
        (stateOverrides = {
          ...
          [`${token}`]: {
            value: {
              ...
              [`_allowances[${userAddress}][${this.reserveAddress}]`]:
                MaxInt256.toString(),
              [`balanceOf[${userAddress}]`]: MaxInt256.toString(),
              [`allowance[${userAddress}][${this.reserveAddress}]`]:
                MaxInt256.toString(),
              [`balances[${userAddress}]`]: MaxInt256.toString(),
              [`allowed[${userAddress}][${this.reserveAddress}]`]:
                MaxInt256.toString(),
            },
          },
        })
