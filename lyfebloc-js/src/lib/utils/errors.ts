const lyfeblocErrorCodes: Record<string, string> = {
  '000': 'ADD_OVERFLOW',
  '001': 'SUB_OVERFLOW',
  '002': 'SUB_UNDERFLOW',
  '003': 'MUL_OVERFLOW',
...
export class LyfeblocErrors {
  ...
  static isErrorCode = (error: string): boolean => {
    if (!error.includes('LYFE#')) return false;

    const errorCode = error.replace('LYFE#', '');
    return Object.keys(lyfeblocErrorCodes).includes(errorCode);
  };

  /**
   * Decodes a Lyfebloc error code into the corresponding reason
   * @param error - a Lyfebloc error code of the form `LYFE#000`
   * @returns The decoded error reason
   */
  static parseErrorCode = (error: string): string => {
    if (!error.includes('LYFE#')) throw new Error('Error code not found');
    const errorCode = error.replace('LYFE#', '');

    const actualError = lyfeblocErrorCodes[errorCode];
  ...
  static tryParseErrorCode = (error: string): string => {
    try {
      return LyfeblocErrors.parseErrorCode(error);
    } catch {
      return error;
    }
  };
  ...
  /**
   * Tests whether a string is a known Lyfebloc error message
   * @param error - a string to be checked verified as a Lyfebloc error message
   */
  static isLyfeblocError = (error: string): boolean =>
    Object.values(lyfeblocErrorCodes).includes(error);

  /**
   * Encodes an error string into the corresponding error code
   * @param error - a Lyfebloc error message string
   * @returns a Lyfebloc error code of the form `LYFE#000`
   */
  static encodeError = (error: string): string => {
    const encodedError = Object.entries(lyfeblocErrorCodes).find(
      ([, message]) => message === error
    );

    if (!encodedError) throw Error('Error message not found');

    return `LYFE#${encodedError[0]}`;
  };
}
