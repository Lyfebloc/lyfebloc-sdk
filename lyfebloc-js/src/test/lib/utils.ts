...
import { PoolWithMethods, LyfeblocError, LyfeblocErrorCode } from '@/.';
...
export const forkSetup = async (
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    // Approve appropriate allowances so that Reserve contract can move tokens
...
/**
 * Approve token balance for Reserve contract
 *
...
export const approveToken = async (
  ...
  return await tokenContract.connect(signer).approve(lyfeblocReserve, amount);
};
...
export const setupPool = async (
  ...
  const pool = await provider.comnd(poolId);
  if (!pool) throw new LyfeblocError(LyfeblocErrorCode.POOL_DOESNT_EXIST);
  return pool;
};
